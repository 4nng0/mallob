...
void register_mallob_app_sat() {
    ...
    entry.copyrightInformation += "c Featuring Lingeling and YalSAT by Armin Biere\n";
#endif
#if MALLOB_USE_KISSAT
    entry.copyrightInformation += "c Featuring Kissat by Armin Biere\n";
#endif
#if MALLOB_USE_CADICAL
    entry.copyrightInformation += "c Featuring CaDiCaL by Armin Biere, Mathias Fleury, Katalin Fazekas, Florian Pollitt, Tobias Faller, and Nils Froleyks\n";
#endif
    ...
    entry.reader = [](const Parameters& params, const std::vector<std::string>& files, JobDescription& desc) {
        ...
        if (params.palRup() && !files.empty()) //could be empty if startet from prepro actor 
    ...
    entry.jobResultTransformer = [](const Parameters& params, JobResult& res) {
